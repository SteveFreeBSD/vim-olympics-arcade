...
function mergeGroups(rawGroups){
  ...
  for(const g0 of rawGroups){
    ...
    const current = map.get(cat) || { category: cat, items: [], _sig: new Set() };
    for(const it of g.items){
      const sig = `${it.keys || ''}__${it.desc || ''}`;
  ...
  const groups = Array.from(map.values()).map(({category, items}) => ({ category, items }));
