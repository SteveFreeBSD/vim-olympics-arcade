...
export default function CommandCard({ item, onOpen }) {
  const hasDeep = !!(item.details || (item.examples && item.examples.length) || item.tutorial);
  return (
    <div className="group rounded-2xl border border-slate-700/70 bg-slate-900/70 transition p-3 flex items-start justify-between gap-3">
      <div>
        <div className="flex items-center gap-2">
          <div className="font-semibold text-slate-100 tracking-tight">{item.keys}</div>
          <span className={"px-2 py-0.5 rounded text-[10px] border " + (hasDeep ? "border-emerald-400 text-emerald-200 bg-emerald-500/10" : "border-slate-600 text-slate-400 bg-slate-800/40")}>
            {hasDeep ? "Deep info" : "Basic"}
          </span>
        </div>
        <div className="text-slate-300 text-sm">{item.desc}</div>
        {item.plugin && <div className="mt-1 text-[11px] text-slate-400">[{item.pluginName}]</div>}
