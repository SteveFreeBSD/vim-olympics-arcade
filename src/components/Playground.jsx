...
const Playground = forwardRef(function Playground(_, ref) {
  ...
  const wordE = () => {
    const s = buf[row] || ''
    let i = Math.max(col, 0)
    if (i < s.length - 1) {
      if (!isWordCh(s[i])) {
        while (i < s.length && /\W/.test(s[i])) i++
      }
      while (i < s.length - 1 && isWordCh(s[i + 1])) i++
    }
    setCol(clamp(i))
  }
  const wordEBack = () => {
  ...
  const wordBoundsAt = (s, i) => {
    let a = i
    if (!isWordCh(s[a]) && a > 0 && isWordCh(s[a - 1])) a = a - 1
  ...
  const awBounds = (s, i) => {
    let [a, b] = wordBoundsAt(s, i)
    while (b < s.length && /\s/.test(s[b])) b++
    return [a, b]
  }
  const betweenDelims = (s, i, chOpen, chClose, include) => {
    // same line only
    let openIdx = -1,
      closeIdx = -1
    for (let k = i; k >= 0; k--) {
  ...
  const onKey = e => {
    ...
    if (recordingReg && !replayingMacro) {
      ...
      setMacroRegs(m => {
        const arr = m[recordingReg] ? [...m[recordingReg]] : []
        arr.push(k)
        return { ...m, [recordingReg]: arr }
      })
    }
    ...
    if (awaitSetMark) {
      if (/^[a-z]$/.test(k)) {
        setMarks(m => ({ ...m, [k]: { row, col } }))
      }
      setAwaitSetMark(false)
      return
    }
    if (awaitJumpMark) {
      if (/^[a-z]$/.test(k)) {
        ...
        if (pos) {
          setRow(pos.row)
          setCol(clamp(pos.col))
        }
      }
      setAwaitJumpMark(null)
      return
    }
    // Insert mode typing
    if (insertMode) {
      ...
      if (k === 'Backspace') {
        ...
        if (col > 0) {
          const nb = [...buf]
          nb[row] = s.slice(0, i) + s.slice(i + 1)
          setBuf(nb)
          setCol(i)
        }
        return
      ...
      if (k.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        const i = Math.max(0, Math.min(col, s.length))
        const nb = [...buf]
        nb[row] = s.slice(0, i) + k + s.slice(i)
        setBuf(nb)
        setCol(i + 1)
    ...
    if (awaitChar) {
      const { op, count } = awaitChar
      const ch = k
      const s = buf[row] || ''
      let target = -1
      let i
      let times = Math.max(1, count || 1)
      if (op === 'f' || op === 't') {
        i = col
        while (times--) {
          ...
          i = idx
          target = op === 'f' ? idx : Math.max(0, idx - 1)
        }
      } else if (op === 'F' || op === 'T') {
        i = col
        while (times--) {
          const idx = s.lastIndexOf(ch, i - 1)
          if (idx < 0) {
            break
          }
          i = idx
          target = op === 'F' ? idx : Math.min(s.length - 1, idx + 1)
        }
      }
      if (target >= 0) {
        setCol(clamp(target))
        recordOp(op + ch, String(awaitChar.count || ''))
      }
      setAwaitChar(null)
      setPending('')
      return
    ...
    if (k === 'x') {
      ...
      recordOp('x', countStr)
      setPending('')
      setLastChange({ kind: 'x', count: cnt })
      return
    }
    if (k === 'u') {
      undo()
    ...
    if (k === 'f' || k === 't' || k === 'F' || k === 'T') {
      setAwaitChar({ op: k, count: Math.max(1, parseInt(countStr || '1', 10)) })
      setPending('')
      return
    }
    ...
    if (k === '@') {
      setAwaitChar({
        op: '@',
        count: Math.max(1, parseInt(countStr || '1', 10)),
      })
      setPending('')
      return
    }
    ...
    if (k === 'a' && !opPending) {
      const s = buf[row] || ''
      setCol(c => Math.min((s || '').length, c + 1))
      setInsertMode(true)
      setInsertFromChange(false)
      setInsertText('')
      setChangeCount(1)
    ...
    if (k === 'o' && !opPending) {
      ...
      nb.splice(row + 1, 0, '')
      setBuf(nb)
      setRow(r => Math.min(nb.length - 1, r + 1))
      setCol(0)
      setInsertMode(true)
      setInsertFromChange(false)
      setInsertText('')
    ...
    if (k === 'd' || k === 'c' || k === 'y') {
      setOpPending({ op: k, count: Math.max(1, parseInt(countStr || '1', 10)) })
      setPending('')
      return
    }
    // Text objects: i?/a?
    if ((k === 'i' || k === 'a') && opPending) {
      setObjPending({ op: opPending.op, kind: k, count: opPending.count || 1 })
      setOpPending(null)
      return
    }
    if (objPending) {
      const { op, kind } = objPending
      const s = buf[row] || ''
      let range = null
      if (k === 'w')
        range = kind === 'a' ? awBounds(s, col) : wordBoundsAt(s, col)
      else if (k === '"') range = betweenDelims(s, col, '"', '"', kind === 'a')
      else if (k === "'") range = betweenDelims(s, col, "'", "'", kind === 'a')
      else if (k === '`') range = betweenDelims(s, col, '`', '`', kind === 'a')
      else if (k === ')' || k === '(')
        range = betweenDelims(s, col, '(', ')', kind === 'a')
      else if (k === ']' || k === '[')
        range = betweenDelims(s, col, '[', ']', kind === 'a')
      else if (k === '}' || k === '{')
        range = betweenDelims(s, col, '{', '}', kind === 'a')
      ...
      if (range) {
        ...
        const start = Math.max(0, a),
          end = Math.max(start, Math.min(b, s.length))
        const left = s.slice(0, start),
          right = s.slice(end)
        if (op === 'y') {
          /* optional: store register */ setObjPending(null)
          return
        }
        nb[row] = left + right
        setBuf(nb)
        setCol(Math.min(start, Math.max(0, (nb[row] || '').length - 1)))
        recordOp(op + kind + k, '')
        if (op === 'c') {
          setInsertMode(true)
          setInsertFromChange(true)
          setInsertText('')
          setChangeCount(1)
          setLastChange(null)
        } else if (op === 'd') {
          setLastChange({ kind: 'dw', count: 1 })
        }
      }
      setObjPending(null)
      return
    ...
    if (k === 'w' && opPending) {
      ...
      for (let t = 0; t < times; t++) {
        const s = nb[r] || ''
        let i = c // delete from current to start of next word boundary
        // advance to end of current word, then to start of next
        if (i < s.length && isWordCh(s[i])) {
          while (i < s.length && isWordCh(s[i])) i++
        }
        while (i < s.length && /\W/.test(s[i])) i++
      ...
      if (wasC) {
        ...
        setInsertFromChange(true)
        setInsertText('')
        setChangeCount(times)
        setLastChange(null)
      } else {
        setLastChange({ kind: 'dw', count: times })
      }
      return
    }
    if (k === 'y' && opPending && opPending.op === 'y') {
      ...
      setReg(lines)
      recordOp('yy', String(opPending.count || ''))
      setOpPending(null)
      return
    }
    if (k === 'p') {
      ...
      pushHist()
      const nb = [...buf]
      nb.splice(row + 1, 0, ...reg)
      setBuf(nb)
      setRow(Math.min(nb.length - 1, row + reg.length))
      recordOp('p', '')
      setPending('')
      setLastChange({ kind: 'p', lines: [...reg] })
      return
    }
    ...
    if (k === '*') {
      ...
      let a = col,
        b = col
      if (!isWordCh(cur[a])) {
        if (a > 0 && isWordCh(cur[a - 1])) a = a - 1
        else {
          recordOp('*', '')
          return
        }
      }
      while (a > 0 && isWordCh(cur[a - 1])) a--
      while (b < cur.length && isWordCh(cur[b])) b++
      const w = cur.slice(a, b)
      if (!w) {
        recordOp('*', '')
        return
      }
    ...
    if (k === '.') {
      ...
      if (ch.kind === 'dw') {
        setOpPending({ op: 'd', count: ch.count || 1 })
        return onKey({ key: 'w', preventDefault: () => {} })
      }
      if (ch.kind === 'cw') {
        ...
        for (let t = 0; t < times; t++) {
          ...
          while (i < s.length && /\\W/.test(s[i])) i++
          const start = Math.min(c, s.length),
            end = Math.min(i, s.length)
          nb[r] = s.slice(0, start) + s.slice(end)
          c = Math.min(start, Math.max(0, (nb[r] || '').length - 1))
        }
        const s2 = nb[r] || ''
        const i2 = Math.max(0, Math.min(c, s2.length))
        nb[r] = s2.slice(0, i2) + (ch.text || '') + s2.slice(i2)
        setBuf(nb)
        setRow(r)
        setCol(i2 + (ch.text || '').length)
        recordOp('.', '')
        return
      }
      if (ch.kind === 'p') {
        pushHist()
        const nb = [...buf]
        nb.splice(row + 1, 0, ...(ch.lines || []))
        setBuf(nb)
        setRow(Math.min(nb.length - 1, row + (ch.lines?.length || 0)))
        recordOp('.', '')
        return
      }
      return
  ...
  return (
    <div className="rounded-3xl border border-slate-700/70 bg-slate-950/70 neo-card">
      <div className="flex flex-col gap-2 border-b border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/80 px-4 py-3">
        <div className="flex items-center justify-between">
          <div className="text-sm text-slate-300">
            ...
            <span className="text-slate-500">
              (hjkl, 0, ^, $, w/W, e/E, b/B, ge, f/t/F/T, *, gg, G, x (count),
              dw/cw, yy, p, u, i, a/A/I/o/O, Enter newline, iw/aw, quotes,
              parens, brackets, braces, angles, marks m &apos;`&apos;, macros
              q/@, . repeat)
            </span>
          </div>
          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-xs text-slate-300">
              ...
              <input
                ...
                step="10"
                value={delay}
                onChange={e => setDelay(parseInt(e.target.value, 10) || 80)}
                className="accent-cyan-400"
              />
              <span className="tabular-nums w-10 text-slate-400">
                {delay}ms
              </span>
            </label>
            {recording ? (
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          ...
          {insertMode && (
            <span className="px-2 py-0.5 rounded border border-emerald-400 text-emerald-200 bg-emerald-500/10">
              Insert
            </span>
          )}
          {replaying && (
            <span className="px-2 py-0.5 rounded border border-cyan-400 text-cyan-200 bg-cyan-500/10">
              Replaying: {replaying}
            </span>
          )}
