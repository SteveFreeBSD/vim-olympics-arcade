...
// external API so App can load scenarios / send keys
let externalApi = {
  setBuffer: (lines) => {},
  sendKeys: (keys) => {}
};
export function getPlaygroundApi(){ return externalApi; }
...
export default function Playground(){
  ...
  const wordF=()=>{const s=buf[row]||"";let i=col+1;while(i<s.length&&/[A-Za-z0-9_]/.test(s[i]))i++;while(i<s.length&&/\\W/.test(s[i]))i++;setCol(clamp(i));};
  const wordB=()=>{const s=buf[row]||"";let i=Math.max(0,col-1);while(i>0&&/\\W/.test(s[i]))i--;while(i>0&&/[A-Za-z0-9_]/.test(s[i-1]))i--;setCol(clamp(i));};
  ...
  externalApi.setBuffer = (lines) => {
    ...
    for(let i=0;i<safe.length;i++){
      ...
      if(idx>=0){ r=i; c=idx; break; }
    }
    setRow(r); setCol(c); setPending("");
  };
  externalApi.sendKeys = async (keys) => {
  ...
  const onKey=e=>{
    ...
    if(k==="0"){setCol(0);recordOp("0","");setPending("");return}
    if(k==="$"){setCol(Math.max(0,(buf[row]||"").length-1));recordOp("$","");setPending("");return}

    if(k==="w")return run(wordF,"w");
    if(k==="b")return run(wordB,"b");

    if(k==="g"){ // potential gg
      if(pending==="g"){ setRow(0); setCol(0); setPending(""); recordOp("gg",""); return; }
      setPending("g"); return;
    }
    if(k==="G"){ setRow(buf.length-1); setCol(0); recordOp("G",""); setPending(""); return; }

    if(k==="x"){
      const s=buf[row]||"";const i=clamp(col);
      if(i>=(s.length))return;
      const ns=s.slice(0,i)+s.slice(i+1);const nb=[...buf];nb[row]=ns;setBuf(nb);setCol(Math.min(i,Math.max(0,ns.length-1)));recordOp("x","");setPending("");
      return;
    }
    if(k==="u"){reset();recordOp("u","");return}
  };
  ...
  const clearRec=()=>{setRecorded([]); setStartBuf(bufferWithCursor())};
  ...
  return(<div className='rounded-3xl border border-slate-700/70 bg-slate-950/70 shadow-[inset_0_1px_0_rgba(255,255,255,.06),0_50px_120px_-60px_rgba(236,72,153,.35)]'>
    <div className='flex flex-col gap-2 border-b border-slate-800/70 bg-gradient-to-b from-slate-900/80 to-slate-950/80 px-4 py-3'>
      <div className='flex items-center justify-between'>
        <div className='text-sm text-slate-300'>
          Motion Playground <span className='text-slate-500'>(hjkl, 0, $, w, b, gg, G, x, u)</span>
        </div>
    ...
    <div tabIndex={0} ref={areaRef} onKeyDown={onKey} className='p-4 font-mono text-[13px] leading-6 outline-none'>
      {buf.map((ln,r)=>(<div key={r} className={'whitespace-pre ' + (r===row?'bg-slate-900/60':'')}>
        <span className='opacity-40 select-none w-8 inline-block text-right mr-2'>{String(r+1).padStart(2," ")}</span>
        {r===row?(<><span>{ln.slice(0,col).replace("|","")}</span><span className='rounded-sm px-0.5 bg-cyan-400 text-slate-900'>{(ln.replace("|",""))[col] || " "}</span><span>{ln.replace("|","").slice(col+1)}</span></>)
                 :(<span>{ln.replace("|","")}</span>)}
      </div>))}
      <div className='mt-3 text-xs text-slate-400'>Click and use hjkl, 0, $, w, b, gg, G. Counts like 3w work. Press Start Recording to capture a lesson.</div>
    </div>
  </div>)
}
